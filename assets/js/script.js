...
(function(){
  ...
  const key='ah-theme'; const apply=t=>document.documentElement.dataset.theme=t;
  const saved = localStorage.getItem(key); if(saved) apply(saved);
  btn.addEventListener('click', ()=>{ const next=(document.documentElement.dataset.theme==='dark')?'light':'dark'; apply(next); localStorage.setItem(key,next); });
})();
...
// Publications helpers (only run on publications page)
...
function renderHighlights(pubs){
  ...
  grid.innerHTML=''; const highlights = pubs.filter(p=>p.highlight);
  if(highlights.length===0){ if(empty) empty.hidden=false; return; } if(empty) empty.hidden=true;
...
async function initPubs(){
  try{
    ...
    renderHighlights(state.pubs); buildYearFilter(state.pubs); applyFilters();
  }catch(e){ document.getElementById('pubsEmpty')?.removeAttribute('hidden'); document.getElementById('highlightsEmpty')?.removeAttribute('hidden'); }
}
...
async function initProjects(){
  try{
    ...
    data.forEach(p=>{
      const el=document.createElement('article'); el.className='card';
      const linkBtns=Object.entries(p.links||{}).filter(([,v])=>v).map(([k,v])=>`<a href="${v}" target="_blank" rel="noopener">${k}</a>`).join(' ');
